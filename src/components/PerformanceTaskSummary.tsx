...
export function PerformanceTaskSummary({
  ...
  // Format success criteria as a list
  const successCriteriaList = task.successCriteria
    ?.split("\n")
    .filter((line) => line.trim().startsWith("-"))
    .map((line, index) => (
      <li key={index} className="ml-4 mt-2">
  ...
  // Format focus topics as a list
  const focusTopicsList = task.suggestedFocusTopic
    ?.split("\n")
    .filter((line) => line.trim().startsWith("-"))
    .map((line, index) => (
      <li key={index} className="ml-4 mt-2">
              ...
              <Tabs defaultValue="overview" className="mt-4">
                ...
                <TabsContent value="rubric" className="p-4 rounded-md bg-gray-50">
                  ...
                  <div className="grid grid-cols-1 gap-4 mt-4">
                    {task.rubricCriteria?.map((criterion) => (
